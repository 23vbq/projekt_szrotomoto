...
if ($offer['created_by'] != $currentUserId) {
    Response::error('Unauthorized to set this offer as removed', Response::HTTP_FORBIDDEN);
    exit;
}

$stmt = Database::getPdo()->prepare('UPDATE offers SET status = :status, updated_at = NOW() WHERE id = :offer_id');
$result = $stmt->execute([
    ':status' => Consts::OFFER_STATUS_REMOVED,
    ':offer_id' => $offerId
]);

if (!$result) {
    Response::error('Failed to set offer as removed', Response::HTTP_INTERNAL_SERVER_ERROR);
    exit;
}
